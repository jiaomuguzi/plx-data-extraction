...
%% 
clear;
plxfiledir = 'Y:\zym\IS\2_control_4\activate\plx\before\';
exportdir = 'Y:\zym\IS\2_control_4\activate\plx\before\';
...
for ii = 1:length(dats)
    ...
    for ch = 1:length(channel)
        ...
        unit_num1 =  repmat(unit_num', 32, 1);
    ...
    for i = 1:length(max_idx)
        ...
        ylabel('10*Voltage(¦ÌV)');
        xticks(0:4:32);
        xticklabels(0:0.1:0.8);
        ...
        s = xlswrite(fullfile([plxfiledir,plxfilename],[output(max_idx(i)).unit, '_waveform_data.xlsx']), mean_waveform_p);
        close;
    end
